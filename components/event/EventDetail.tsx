...
function Row({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="grid grid-cols-[140px_1fr] gap-4 py-3 text-[14px]">
      ...
      <dd className="min-w-0 max-w-3xl break-words text-ink">{children}</dd>
    </div>
  );
}
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mt-8">
      ...
      <div className="mt-2 max-w-3xl text-[15px] leading-7 text-ink">{children}</div>
    </section>
  );
}
...
export function EventDetail({ eventId }: { eventId: string }) {
  ...
  return (
    <article aria-labelledby="event-title">
      ...
      {/* Wide screens: narrative beside the factual cards; stacked below xl. */}
      <div className="xl:grid xl:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] xl:items-start xl:gap-x-12">
        ...
        <div className="mt-10 xl:mt-5">
          <section className="rounded-(--radius-card) border border-line bg-surface px-5">
